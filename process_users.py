...
def create_user(name: str, uuid=None) -> dict:
    ...
    if not uuid:
        uuid = subprocess.run(
            './xray uuid', shell=True, capture_output=True, text=True
        ).stdout.strip()
    user['id'] = uuid
    user['alterId'] = 0
    user['email'] = name
...
def create_user_info(
    client_data, config_data, quota, days, concurrent_connections
):
    """
...
def add_user_to_file(
    ...
    create_user_info(
        client_dict, file_data, quota, days, concurrent_connections
    )
    ...
    this_month_user_stats = os.path.join(
        'users', user, 'Statistics', current_date[0], current_date[1]
    )
    today_user_stats = os.path.join(
        this_month_user_stats, current_date[2] + '.json'
    )
...
def refresh_user_in_file(
    ...
    send_log(
        LOG_PATH, f"added '{user}' softly to '{config_file_path}' file.\n"
    )
...
def del_user_from_file(name: str, config_file_path) -> bool:
    ...
    if user_exists:
        send_log(
            LOG_PATH, f"removed '{name}' from '{config_file_path}' file.\n"
        )
    return user_exists
